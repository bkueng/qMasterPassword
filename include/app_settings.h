...
class ApplicationSettings {
   public:
    ...
    {
        show_systray_icon =
            settings.value("app_settings/show_systray_icon", show_systray_icon).toBool();
        show_pw_after_login =
            settings.value("app_settings/show_password_after_login", show_pw_after_login).toBool();
        auto_logout_when_hidden =
            settings.value("app_settings/auto_logout_when_hidden", auto_logout_when_hidden)
                .toBool();
        auto_logout_timeout =
            settings.value("app_settings/auto_logout_timeout", auto_logout_timeout).toInt();
        clipboard_pw_timeout =
            settings.value("app_settings/clipboard_pw_timeout", clipboard_pw_timeout).toInt();
        show_identicon = settings.value("app_settings/show_identicon", show_identicon).toBool();
        form_fill_hide_window =
            settings.value("app_settings/form_fill_hide_window", form_fill_hide_window).toBool();
    }
