...
void CCommandLineParser::addParam(const string& name, char short_name, const string& default_val, const string& task_name) {
	map<string, SCLParam>* param=&m_global.params;
	if(task_name.length()>0) {
		map<string, SCLTask>::iterator iter=m_tasks.find(task_name);
		ASSERT_THROW_e(iter!=m_tasks.end(), EINVALID_PARAMETER, "Task name %s not found", task_name.c_str());
