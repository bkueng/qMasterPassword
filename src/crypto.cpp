...
{
    if (type_str == "GeneratedMaximum")
        m_type = MPSiteTypeGeneratedMaximum;
    else if (type_str == "GeneratedLong")
        m_type = MPSiteTypeGeneratedLong;
    else if (type_str == "GeneratedMedium")
        m_type = MPSiteTypeGeneratedMedium;
    else if (type_str == "GeneratedBasic")
        m_type = MPSiteTypeGeneratedBasic;
    else if (type_str == "GeneratedShort")
        m_type = MPSiteTypeGeneratedShort;
    else if (type_str == "GeneratedPIN")
        m_type = MPSiteTypeGeneratedPIN;
    else if (type_str == "GeneratedName")
        m_type = MPSiteTypeGeneratedName;
    else if (type_str == "GeneratedPhrase")
        m_type = MPSiteTypeGeneratedPhrase;
    else
...
{
    if (variant == "Password")
        m_variant = MPSiteVariantPassword;
    else if (variant == "Login")
        m_variant = MPSiteVariantLogin;
    else if (variant == "Answer")
        m_variant = MPSiteVariantAnswer;
    else
        THROW(EINVALID_PARAMETER);
}
...
std::string User::hash(const string& password, const string& salt)
{
    ...
    int ret =
        scrypt((const uint8_t*)password.c_str(), password.length(), (const uint8_t*)salt.c_str(),
               salt.length(), MP_N, MP_r, MP_p, hash_buffer, hash_length);
...
bool MasterPassword::login(const User& user, const std::string& password)
{
    logout();
    ...
    int ret = scrypt((const uint8_t*)password.c_str(), password.length(),
                     (const uint8_t*)master_key_salt.c_str(), master_key_salt.length(), MP_N, MP_r,
                     MP_p, m_master_key, MP_dkLen);

    if (user.storePasswordHash()) {
        if (!hash.valid()) throw CryptoException(CryptoException::Type_thread_exception);
        if (hash.get() != user.getPasswordHash()) return false;
    }
...
{
    for (int i = 0; i < MP_dkLen; ++i) m_master_key[i] = 0;
    m_is_logged_in = false;
}

std::string MasterPassword::sitePassword(const Site& site)
{
    if (!m_is_logged_in) throw CryptoException(CryptoException::Type_not_logged_in);
    ...
    const unsigned char* ret =
        HMAC_SHA256(m_master_key, MP_dkLen, (const uint8_t*)site_password_seed_info.c_str(),
                    site_password_seed_info.length(), site_password_seed);
    if (!ret) throw CryptoException(CryptoException::Type_HMAC_SHA256_failed);

    // Determine the template.
    string templates = templateForType(site.m_type, site_password_seed[0]);
    if (templates.length() > seed_len - 1) {
        THROW_s(EINVALID_PARAMETER, "template too long for password seed (%i)", templates.length());
    }
...
std::string MasterPassword::templateForType(MPSiteType type, uint8_t seed_byte)
{
    ...
    switch (type) {
        ...
        case MPSiteTypeGeneratedPhrase: {
            const char* templates[] = {"cvcc cvc cvccvcv cvc", "cvc cvccvcvcv cvcv",
...
char MasterPassword::characterFromClass(char character_class, uint8_t seed_byte)
{
