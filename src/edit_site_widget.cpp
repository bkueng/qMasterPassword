...
EditSiteWidget::EditSiteWidget(const QMap<CategoryId, QString>& categories, UiSite& site, Type type,
                               QWidget* parent)
    : QDialog(parent),
      ui(new Ui::EditSiteWidget),
      m_site(site),
      m_type(type),
...
void EditSiteWidget::applyData()
{
    ...
    for (int i = 0; i < ui->layoutCategories->count(); ++i) {
        CategoryCheckbox* checkbox =
            dynamic_cast<CategoryCheckbox*>(ui->layoutCategories->itemAt(i)->widget());
