...
Exception::Exception(EnErrors err)
    : m_bLogged(false), m_func(NULL), m_file(NULL), m_line(0), m_err(err)
{
}
...
ExceptionString::ExceptionString(EnErrors err, const char* func, const char* file, int line,
                                 const char* fmt, ...)
...
{
    if (!m_bLogged) {
        CLog::getInstance().Log(LogLevel::Error, m_file, m_func, m_line, "%s", m_err_desc.c_str());
        m_bLogged = true;
    }
}
