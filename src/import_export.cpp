...
DataImportExport::DataImportExport(const QMap<CategoryId, QString>& categories)
    : m_categories(categories)
{
}

void DataImportExport::exportJson(const UiUser& user, const QString& file_name)
{
    ...
    for (const auto& site : user.getSites()) {
        ...
        site_object["passwordType"] =
            "Generated" + QString::fromUtf8(MPSiteTypeToString(site->site.getType()).c_str());
        ...
        if (site->url != "") site_object["url"] = site->url;
        if (site->comment != "") site_object["comment"] = site->comment;
        sites_array.append(site_object);
    }
    QJsonDocument save_doc(sites_array);
    if (save_file.write(save_doc.toJson()) == -1) THROW(EFILE_ERROR);
}
...
void DataImportExport::importJson(UiUser& user, const QString& file_name)
{
    ...
    for (const auto& json_site_iter : json_sites_array) {
        ...
        if (site_name != "") {
            ...
            // apply data
            for (auto iter = m_categories.constBegin(); iter != m_categories.constEnd(); ++iter) {
