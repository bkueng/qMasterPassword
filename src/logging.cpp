...
bool CLog::parseLevel(const string& level, LogLevel& level_out)
{
    ...
    if (str == "error" || str == "e")
        level_out = LogLevel::Error;
    else if (str == "warn" || str == "w")
        level_out = LogLevel::Warn;
    else if (str == "info" || str == "i")
        level_out = LogLevel::Info;
    else if (str == "debug" || str == "d")
        level_out = LogLevel::Debug;
    else if (str == "none" || str == "n")
        level_out = LogLevel::None;
    else
        return false;
    return true;
}

void CLog::Log(LogLevel level, const char* file, const char* function, int line, const char* fmt,
               ...)
{
    char buffer[2048];
    va_list args;
    ...
    if (level <= m_console_log) {
        if (level == LogLevel::Error) {
            ...
            if (m_bLog_time_console)
                fprintf(stderr, "%s %s: ", getDate().c_str(), getTime().c_str());
            fprintf(stderr, "%s\n", buffer);
        } else {
            if (m_bLog_src_file[static_cast<int>(level)] && file) {
#ifdef _DEBUG
...
string CLog::getDate()
{
    ...
    sprintf(timestr, "%02i.%02i.%02i", (int)ptm->tm_mday, (int)ptm->tm_mon + 1,
            (int)ptm->tm_year % 100);
    return timestr;
}
string CLog::getTime()
{
    ...
    time_t seconds = time(0);
    struct tm* ptm = localtime(&seconds);
    sprintf(timestr, "%02i:%02i:%02i", (int)ptm->tm_hour, (int)ptm->tm_min, (int)ptm->tm_sec);
    return timestr;
}

CLog::CLog()
    : m_bLog_time_file(true),
      m_bLog_time_console(false),
      m_console_log(LogLevel::Info),
      m_file_log(LogLevel::Info)
{
