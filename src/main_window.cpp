...
DBusAdapter::DBusAdapter(MainWindow* main_window) : QObject(main_window), m_main_window(main_window)
{
}
...
MainWindow::MainWindow(QWidget* parent)
    : QMainWindow(parent), m_ui(new Ui::MainWindow), m_import_export(m_categories)
{
    m_ui->setupUi(this);
    ...
    connect(m_clipboard_timer, SIGNAL(timeout()), this, SLOT(clearDataFromClipboardTimer()));
    ...
    connect(m_delayed_identicon_update_timer, SIGNAL(timeout()), this,
            SLOT(delayedIdenticonUpdate()));
    ...
    m_table_shortcuts[(int)ShortcutAction::CopyPWToClipboard].push_back(QKeySequence(Qt::Key_Y));
    m_table_shortcuts[(int)ShortcutAction::CopyUserToClipboard].push_back(QKeySequence(Qt::Key_U));
    ...
    m_table_shortcuts[(int)ShortcutAction::NextItem].push_back(QKeySequence(Qt::Key_J));
    m_table_shortcuts[(int)ShortcutAction::OpenURL].push_back(QKeySequence(Qt::Key_O));
    m_table_shortcuts[(int)ShortcutAction::Logout].push_back(QKeySequence(Qt::Key_Q));
...
void MainWindow::initSitesView()
{
    ...
    m_ui->tblSites->horizontalHeader()->setSectionResizeMode(m_copy_column_idx, QHeaderView::Fixed);
...
void MainWindow::initTrayIcon()
{
    ...
    connect(m_tray_icon, SIGNAL(activated(QSystemTrayIcon::ActivationReason)), this,
...
void MainWindow::updateModelItem(int row, const UiSite& site)
{
    ...
    // update the buttons
    m_ui->tblSites->closePersistentEditor(
        m_proxy_model->mapFromSource(m_sites_model->index(row, m_copy_column_idx)));
    m_ui->tblSites->openPersistentEditor(
        m_proxy_model->mapFromSource(m_sites_model->index(row, m_copy_column_idx)));
    ++i;  // copy button
    m_sites_model->item(row, i++)->setText(site.comment);
    m_sites_model->item(row, i++)->setText(
        QString::fromUtf8(MPSiteTypeToString(site.site.getType()).c_str()));
...
void MainWindow::login()
{
    ...
    if (user_name.length() == 0 || password.length() == 0 || iter_user == m_users.end()) return;

    try {
        m_current_user = &iter_user.value();
        m_master_password.logout();
        bool logged_in =
            m_master_password.login(m_current_user->userData(), password.toUtf8().constData());
        if (!logged_in) {
        ...
        if (m_proxy_model->rowCount() > 0) m_ui->tblSites->selectRow(0);

        // clear the password: no need to store it anymore
        m_ui->txtPassword->setText("");
        for (int i = 0; i < password.length(); ++i) password[i] = '\0';
        m_ui->txtFilter->setFocus();
        if (m_application_settings.show_identicon) m_hide_identicon_timer->start(2000);
        ...
        QMessageBox::critical(this, tr("Cryptographic exception"),
                              tr("Error: %1.\n"
                                 "Should it happen again, then something is seriously wrong with "
...
void MainWindow::logout()
{
    ...
    if (m_clipboard_timer->isActive()) clearDataFromClipboard();
    enableUI(false);
    clearSitesUI();
    m_ui->txtPassword->setFocus();
}
...
void MainWindow::addUser()
{
    ...
    if (add_user.exec() == 1) {  // accepted
        ...
        if (m_users.find(user_name) != m_users.end()) {
            QMessageBox::critical(this, tr("User exists"), tr("Error: user already exists."));
            return;
        }
...
void MainWindow::editUser()
{
    ...
    QString user_name = m_ui->cmbUserName->currentText();
    auto iter_user = m_users.find(user_name);
    if (iter_user == m_users.end()) return;
...
void MainWindow::deleteUser()
{
    ...
    if (selected >= 0) {
        ...
        reply = QMessageBox::question(
            this, tr("Delete User"), tr("Do you really want to delete the user %1?").arg(user_name),
                ...
                if (key_shortcut.matches(key_sequence) == QKeySequence::ExactMatch) {
                    switch ((ShortcutAction)i) {
                        case ShortcutAction::CopyPWToClipboard: {
                            ...
                            if (item) copyPWToClipboard(item->site());
                        } break;
                        case ShortcutAction::CopyUserToClipboard: {
                            TableItem* item = getSelectedItem();
                            if (item) copyUserToClipboard(item->site());
                        } break;
#ifndef DISABLE_FILL_FORM_SHORTCUTS
...
void MainWindow::readSettings()
{
    ...
    if (m_categories.isEmpty()) {
        ...
        for (const auto& category : categories) m_categories[m_next_category_id++] = category;
    } else {
        m_next_category_id = 0;
        for (auto category : m_categories.keys()) {
            if (category > m_next_category_id) m_next_category_id = category;
        }
        ++m_next_category_id;
    ...
    for (auto& user : m_users) {
        LOG(LogLevel::Debug, "Read user: %s (%i sites)", user.getUserName().toUtf8().constData(),
            user.getSites().count());

        m_ui->cmbUserName->addItem(user.getUserName());
        if (user.getUserName() == selected_user) selected_index = m_ui->cmbUserName->count() - 1;
    }
    if (selected_index != -1) m_ui->cmbUserName->setCurrentIndex(selected_index);
    if (!m_users.isEmpty()) m_ui->txtPassword->setFocus();
...
void MainWindow::deleteSite()
{
    ...
    for (auto iter = m_current_user->getSites().begin(); iter != m_current_user->getSites().end();
         ++iter) {
...
QModelIndex MainWindow::getSelectedRow()
{
    ...
    if (selected_rows.count() == 0) return QModelIndex();
    return selected_rows.at(0);
}
TableItem* MainWindow::getSelectedItem()
{
    QModelIndex selected_row = getSelectedRow();
    if (!selected_row.isValid()) return nullptr;
    return dynamic_cast<TableItem*>(
        m_sites_model->itemFromIndex(m_proxy_model->mapToSource(selected_row)));
}
...
void MainWindow::removeCategory(CategoryId category_id)
{
    ...
    for (int i = 0; i < m_ui->layoutCategories->count(); ++i) {
        CategoryButton* button =
            dynamic_cast<CategoryButton*>(m_ui->layoutCategories->itemAt(i)->widget());
...
{
    m_proxy_model->setFilterRegularExpression(QRegularExpression(
        QRegularExpression::escape(filter_text), QRegularExpression::CaseInsensitiveOption));
...
int MainWindow::copyToClipboard(const QString& str)
{
    ...
    if (m_application_settings.clipboard_pw_timeout > 0) {
        if (!m_clipboard_timer->isActive()) m_clipboard_previous_data = original_text;
...
void MainWindow::uiSitesTableChanged()
{
    ...
    for (int row = 0; row < m_proxy_model->rowCount(); ++row) {
        m_ui->tblSites->openPersistentEditor(m_proxy_model->index(row, m_copy_column_idx));
    }
...
{
    if (!m_master_password.isLoggedIn() || !m_application_settings.auto_logout_when_hidden) return;
...
{
    SettingsWidget settings_widget(m_application_settings, m_users, m_categories, m_import_export,
                                   this);
    connect(&settings_widget, SIGNAL(showTrayIconChanged(bool)), this, SLOT(showTrayIcon(bool)));
    ...
    if (settings_widget.exec() == 1) {
        ...
        for (const auto& category_id : m_categories.keys()) {
            ...
            if (!found) removeCategory(category_id);
        }
...
void MainWindow::showAboutWidget()
{
    ...
    QMessageBox::about(this, tr("About %1").arg(APP_NAME),
                       tr(about_msg)
                           ...
                           .arg("<a "
                                "href=\"https://github.com/bkueng/qMasterPassword\">github.com/"
                                "bkueng/qMasterPassword</a>"));
}
                                              ...
                                              const QModelIndex& source_parent) const
{
    if (m_main_window.selectedCategory() != 0) {
        ...
        if (item) {
            if (!item->site().category_ids.contains(m_main_window.selectedCategory())) return false;
        }
    }
...
QString MainWindow::description(ShortcutAction action)
{
    switch (action) {
        ...
        case ShortcutAction::FillForm:
            return tr(
                "Auto fill form: select next window and fill in user name (if set) and password");
        case ShortcutAction::FillFormPasswordOnly:
