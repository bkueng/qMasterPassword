...
PasswordGeneratorWidget::PasswordGeneratorWidget(QWidget* parent)
    : QDialog(parent), ui(new Ui::PasswordGeneratorWidget)
{
    ui->setupUi(this);
...
void PasswordGeneratorWidget::uiChanged()
{
        ...
        case Method::Template: {
            ...
            for (int j = 0; j < (int)templates.size(); ++j) {
                ...
                for (int i = 0; i < templates[j].length(); ++i) {
                    chars = QString::fromStdString(
                        MasterPassword::charactersFromClass(templates[j][i].toLatin1()));
                    num *= chars.length();
                }
                entropy_bits += num;
            }
    ...
    if (entropy_bits < 28.) {
        ...
        ui->lblStrength->setText(
            tr("Weak; should keep out most people, often good for desktop login passwords"));
    } else if (entropy_bits < 60.) {
        ui->lblStrength->setText(
            tr("Reasonable; fairly secure passwords for network and company passwords"));
...
void PasswordGeneratorWidget::addUniqueCharsToString(QString& str, const QString& chars_to_add)
{
    for (int i = 0; i < chars_to_add.length(); ++i) {
        if (str.indexOf(chars_to_add[i]) == -1) str += chars_to_add[i];
    }
}
...
void PasswordGeneratorWidget::getRandomBytes(unsigned char* buffer, int num)
{
    ...
    if (ret == 0)
        LOG(LogLevel::Warn, "Used less secure pseudo-random function for random numbers!");
    else if (ret == -1) {
    ...
    try {
        switch (selectedMethod()) {
            ...
            case Method::Template: {
                ...
                unsigned char* rand_values = new unsigned char[length];
                getRandomBytes(rand_values, length);
                int temp_idx = (int)rand_values[0] | (((int)rand_values[1]) << 8) |
                               (((int)rand_values[2]) << 16) | (((int)rand_values[3]) << 24);
                QString chosen_template = templates[temp_idx % templates.size()];
                QString chars;
                for (int i = 0; i < chosen_template.length(); ++i) {
                    chars = QString::fromStdString(
                        MasterPassword::charactersFromClass(chosen_template[i].toLatin1()));
...
void PasswordGeneratorWidget::generateTemplates(std::vector<QString>& templates_out, int length,
                                                int special_signs, int numbers)
{
    ...
    generateTemplatesRec(templates_out, initial, length - modulo, special_signs, numbers);
    generateTemplatesRec(templates_out, initial, length - modulo, special_signs, numbers);
}

void PasswordGeneratorWidget::generateTemplatesRec(std::vector<QString>& templates_out,
                                                   QString prefix, int length, int special_signs,
                                                   int numbers)
{
    if (length < 0) return;
    ...
    if (special_signs > 0 && numbers > 0) {
        generateTemplatesRec(templates_out, prefix + "noCvcc", length - 6, special_signs - 1,
                             numbers - 1);
        generateTemplatesRec(templates_out, prefix + "noCvcv", length - 6, special_signs - 1,
                             numbers - 1);
        generateTemplatesRec(templates_out, prefix + "onCvcc", length - 6, special_signs - 1,
                             numbers - 1);
        generateTemplatesRec(templates_out, prefix + "onCvcv", length - 6, special_signs - 1,
                             numbers - 1);
    }
    // add templates where special symbol and number is not necessarily together
    if (special_signs > 0) {
        generateTemplatesRec(templates_out, prefix + "oCvcc", length - 5, special_signs - 1,
                             numbers);
        generateTemplatesRec(templates_out, prefix + "oCvcv", length - 5, special_signs - 1,
                             numbers);
    }
    if (numbers > 0) {
        generateTemplatesRec(templates_out, prefix + "nCvcc", length - 5, special_signs,
                             numbers - 1);
        generateTemplatesRec(templates_out, prefix + "nCvcv", length - 5, special_signs,
                             numbers - 1);
    }
...
void PasswordGeneratorWidget::showMethodExplanation()
{
    QWhatsThis::showText(QCursor::pos(),
                         tr("<p><b>Method</b><br>"
                            "The character-based method is purely random for the chosen characters "
                            "and thus harder to guess.<br>"
                            "The template-based method on the other hand is a bit less random but "
                            "its passwords are easier to remember."
                            "</p>"
                            "<p><b>Password Strength</b><br>"
                            "Shows the entropy bits, where an increase of 1 means twice the amount "
                            "of brute-force attempts is needed.<br>"
                            "Note that the statement holds for general passwords. "
                            "A master password in qMasterPassword is harder to crack (compared to "
                            "for example web passwords), "
