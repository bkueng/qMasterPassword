...
SettingsWidget::SettingsWidget(ApplicationSettings& settings, QMap<QString, UiUser>& users,
                               QMap<CategoryId, QString> categories,
                               DataImportExport& import_export, QWidget* parent)
    : QDialog(parent),
      ui(new Ui::SettingsWidget),
      m_settings(settings),
      m_users(users),
      m_import_export(import_export)
{
    ui->setupUi(this);
    ...
    for (const auto& category : categories.keys()) {
        if (category != 0) ui->cmbCategories->addItem(categories[category]);
    }
...
void SettingsWidget::exportAsJsonClicked()
{
    ...
    if (dialog.exec()) {
        ...
        if (!file_name.endsWith(".json", Qt::CaseInsensitive)) file_name += ".json";
        try {
...
UiUser* SettingsWidget::selectedUser()
{
    ...
    if (iter == m_users.end()) return nullptr;
    return &*iter;
}
...
bool SettingsWidget::canAddCategory(const QString& category_name)
{
    ...
    for (int i = 0; i < ui->cmbCategories->count(); ++i) {
        if (ui->cmbCategories->itemText(i) == category_name) can_add = false;
    }
    return can_add;
}
...
{
    while (ui->cmbCategories->count() > 0) ui->cmbCategories->removeItem(0);
