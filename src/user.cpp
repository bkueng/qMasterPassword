...
QDataStream& operator>>(QDataStream& stream, UiSite& site)
{
    ...
    if (version == 1) stream >> site.url;
    stream >> site.comment >> context >> counter >> name >> type >> site.user_name >> variant >>
        site.category_ids >> site.time_created >> site.time_edited;
...
QDataStream& operator>>(QDataStream& stream, UiUser& user)
{
    ...
    if (version == 1) {
        ...
        if (store_hash) {
            ...
            stream >> salt_array;
            user.userData().setStoredHashData(string(hash_array.constData(), hash_array.length()),
