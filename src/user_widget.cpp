...
UserWidget::UserWidget(Type type, QWidget* parent) : QDialog(parent), ui(new Ui::UserWidget)
{
    ui->setupUi(this);
    checkInputValidity();
...
void UserWidget::applyData(UiUser& user)
{
    if (ui->chkCheckPassword->isChecked()) {
        try {
            user.userData().setStorePasswordHash(ui->txtPassword->text().toUtf8().constData());
        } catch (CryptoException& e) {
            QMessageBox::critical(
                this, tr("Cryptographic exception"),
...
void UserWidget::checkInputValidity()
{
    ...
    bool is_valid = ui->txtUserName->text().length() > 0 && password.length() > 0 &&
...
void UserWidget::identiconHelp()
{
    QWhatsThis::showText(QCursor::pos(),
                         tr("<p>The identicon is a visual help calculated from the user name and "
                            "the master password.</p>"
                            "<p>Remember it and if it is the same when logging in, you most likely "
                            "didn't make a typo.</p>"
                            "<p>This avoids the need to explicitly store the hash of the password "
                            "(password check option below).</p>"
                            "<p>It can be disabled under Edit -> Settings.</p>"),
                         this);
}
