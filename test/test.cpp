...
UnitTests::UnitTests(const std::string& file_name) : m_file_name(file_name)
{
}
...
void UnitTests::runTests_data()
{
    ...
    for (auto iter = test_cases.constBegin(); iter != test_cases.constEnd(); ++iter) {
        ...
        QTest::newRow(test_id.toUtf8().constData())
            << user_name << master_password << site_name << site_type << site_counter
            << site_variant << site_context << result;
    }
}
bool UnitTests::getAttribute(const DataTree& data, DataTree::ConstIterator iter, QString name,
                             QString& ret)
{
